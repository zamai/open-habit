...
xcodebuild -project OpenHabit.xcodeproj -scheme OpenHabit -destination "platform=iOS Simulator,id=$DEVICE" -derivedDataPath "$RUNNER_TEMP/tests" -resultBundlePath "$RUNNER_TEMP/tests.xcresult" -only-testing:OpenHabitTests -only-testing:OpenHabitUITests/SharedHabitJoinUITests -only-testing:OpenHabitUITests/DeleteConfirmationUITests test
